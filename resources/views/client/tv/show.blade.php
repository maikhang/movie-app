...
@section('content')
   ...
   <div class="video-container iq-main-slider">
      <iframe class="video d-block" src="https://www.youtube.com/embed/{{$tv['videos']['results'][0]['key']}}" frameborder="0" allow="" allowfullscreen></iframe>
   </div>
   @else
       ...
       <div class="row">
          <div class="col-lg-12">
             <div class="trending-info g-border">
                <h1 class="trending-text big-title text-uppercase mt-0">{{ $tv['name'] }}</h1>
                ...
                <div class="d-flex align-items-center text-white text-detail">
                  <span class="badge badge-secondary p-3">{{ $tv['last_episode_to_air']['vote_average']  * 10 .'%' }}</span>
                  <span class="ml-3">{{ $tv['number_of_episodes'] }} Episodes(
                     @php($runtime = $tv['episode_run_time'][0])
                     @if($runtime >= 60)
                  ...
                  <span class="trending-year">
                     @if(isset($tv['first_air_date']))
                     {{Carbon\Carbon::parse($tv['first_air_date'])->format('M d, Y')}}
                     @endif
                  </span>
                </div>
                <div class="d-flex align-items-center series mb-4">
                  @foreach ($tv['production_companies'] as $logo)
                     @if ($logo['logo_path'])
                ...
                <p class="trending-dec w-100 mb-0">{{ $tv['overview'] }}</p>
                <ul class="list-inline p-0 mt-4 share-icons music-play-lists">
                  <li><span><i class="ri-add-line"></i></span></li>
                  <li><span><i class="ri-heart-fill"></i></span></li>
                  <a href="{{$tv['homepage']}}" target="_blank" class="share-ico">
                     <li>
